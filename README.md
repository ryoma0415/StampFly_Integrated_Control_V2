...
data_analysis/       FF 係数抽出 CLI(make_ff_profile.py)+受入テスト(独立 venv)
...
- **ログ保存**: トグル ON の間だけ `logs/YYYYMMDD_HHMMSS_<mode>.csv`
  ...
  [docs/LOG_STRUCTURE.md](docs/LOG_STRUCTURE.md)(v2: 94列。ヨー指令・
  ヨー推定・EKF 診断・軌道状態を含む)。experiment モード中は 50Hz 送信が
  停止しているため行は記録されない。
...
# FF 抽出の受入テスト(6/12 照合+付録A再現。pytest 不要)
