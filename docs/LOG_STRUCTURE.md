...
- 記録中は CMD_SETPOINT(複数機は各機の CMD_SETPOINT/CMD_POS_ERR)の
  送信(50Hz)ごとに1行を出力する。
...
- 複数機モードでは機体ごとの FlightLogger に `mode="multi"` で記録される。
  ...
  stop_all / 緊急停止後も着陸検知までは記録が続く。取得できない列
  (PID 成分の一部など)は空欄になる。
...
| 列 | 型 | 説明 |
| --- | --- | --- |
| `data_valid` | 0/1 | PID 更新に有効と判断されたか(信頼度・外れ値・トラッキング・フレーム間隔・再シード検疫を考慮)。 |
