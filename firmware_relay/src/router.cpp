...
uint32_t s_uplink_refused = 0;  // ターゲット未設定で拒否した上りフレーム数
...
void forward_uplink(const stampfly::FrameView& f) {
  if (!espnow_link::has_target()) {
    ...
    // CMD_SETPOINT は50Hzで届くため、警告はレート制限して発行する。
    ++s_uplink_refused;
    ...
    if (!s_no_target_warned ||
      ...
      send_log("uplink refused: no target (send RLY_SET_TARGET first, %lu dropped)",
...
void route_espnow_frame(const uint8_t* logical, size_t len) {
  // 受信cbで検証済み。type は論理フレームのオフセット1(PROTOCOL.md)。
  const uint8_t type = logical[1];
  if (stampfly::is_downlink_type(type)) {  // LOG_TEXT(0x40)もこのレンジに含まれる
  ...
  if (stampfly::is_relay_type(frame.type)) {
    switch (static_cast<stampfly::MsgType>(frame.type)) {
      ...
      case stampfly::MsgType::RLY_PING:
        ...
        break;
      default:
        // PC発の ACK/STATS/PONG や未知のリレー型(0x55–0x5F)は処理しない
        break;
    }
    return;
  }
...
void poll(uint32_t now_ms) {
  ...
  for (int i = 0; i < relay_config::ESPNOW_DRAIN_MAX_PER_POLL; ++i) {
    if (!espnow_link::receive(frame, sizeof(frame), &frame_len)) break;
    route_espnow_frame(frame, frame_len);
  }
