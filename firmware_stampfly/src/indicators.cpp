...
constexpr uint32_t COLOR_MOTOR_TEST = 0xff8800;   // MOTOR_TEST(v2: ベンチ実験中の注意喚起)
// 計測中(Experiment記録中)のマゼンタ常灯。動画のカット点検出用に「変化の瞬間」
// を明確にするため点滅させない。COLOR_PURPLE(INIT/CALIBRATION)と同値だが、
// 本オーバーレイは MOTOR_TEST 中のみ有効であり両状態は同時に成立しない
// (INIT/CALIBRATION は起動直後のみ。計測ウィンドウ内でマゼンタ=計測中が一意)。
constexpr uint32_t COLOR_RECORDING = 0xff00ff;
...
// CMD_LED_MODE(計測中インジケータ)の状態。400Hzループ(indicators_update /
// indicators_set_led_mode とも同一コンテキスト)からのみ触るため排他は不要。
bool recording_mode = false;         // RECORDING 要求中か
uint32_t recording_last_ms = 0;      // 最後に mode=1 を受けた時刻(キープアライブ)
...
// RECORDING オーバーレイのフェイルセーフ処理。MOTOR_TEST 離脱で即 AUTO、
// 最後の CMD_LED_MODE(1) から一定時間でも AUTO(リンク断で常灯が残らないように)。
void service_recording_mode(AutoFlightState st) {
    if (!recording_mode) return;
    if (st != AUTO_MOTOR_TEST ||
        millis() - recording_last_ms > FLIGHT_CONFIG.led_recording_failsafe_ms) {
...
void update_leds(void) {
    ...
    switch (st) {
        ...
        case AUTO_MOTOR_TEST: {
            if (recording_mode) {
                // 計測中(CMD_LED_MODE=1): マゼンタ常灯。「マゼンタに変わった瞬間
                // = 計測開始(t_s=0)= スマホ動画のカット位置」という同期規約。
                set_onboard(COLOR_RECORDING, COLOR_RECORDING);
                break;
            }
            // v2 モーターテスト: 橙点滅(プロペラ回転あり得ることの注意喚起)
            const uint32_t c = blink_on() ? COLOR_MOTOR_TEST : 0x000000;
            set_onboard(c, c);
            break;
        }
    }
...
void indicators_set_led_mode(uint8_t mode) {
    using stampfly::CmdLedMode;
    if (mode == CmdLedMode::MODE_RECORDING) {
        // mode=1 の再送はキープアライブ(タイムスタンプ更新のみ)
        recording_mode = true;
        recording_last_ms = millis();
    } else {
        recording_mode = false;
    }
}
